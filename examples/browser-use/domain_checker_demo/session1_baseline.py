...
# Fixed site policy (kept simple for baseline; no ACE/adaptation here)
...
# simple alias matches we might see in redirects
...
class DomainCheckerBaseline:
    ...
    def __init__(
        ...
        self.metrics: Dict[str, Any] = {
            'session': 1,
            'session_type': 'baseline_fixed_pipeline',
            'start_time': None,
            'end_time': None,
            ...
            'average_steps_per_domain': 0.0,  # <-- added for fair comparison
            'model': model,
            'sites_policy': {
                'preferred': list(PREFERRED_SITES),
                'avoided': list(AVOIDED_SITES),
            }
        }
    ...
    def _create_task_prompt(self, domain: str) -> str:
        """
        Fixed pipeline prompt mirroring Session 2’s site selection (no ACE).
        """
        preferred_lines = "\n".join([f"- {s}" for s in PREFERRED_SITES])
        avoided_lines = "\n".join([f"- {s}" for s in AVOIDED_SITES])

        return f"""
You are a domain availability checker agent. Determine if "{domain}" is available.

USE THESE LOOKUP SITES (try in this order, stop after a conclusive result):
{preferred_lines}

AVOID these sites (high chance of CAPTCHA / Access Denied):
{avoided_lines}

INSTRUCTIONS:
1) Navigate to the first preferred site.
2) Find the domain search box.
3) Enter exactly: {domain}
4) Submit the search and wait for the results to fully load.
5) Decide the status from clear indicators such as: "Available", "Add to Cart", "Taken", "Unavailable", "Premium", "Already Registered".
6) If inconclusive or blocked, proceed to the next preferred site.
7) Double-check the result corresponds to the exact domain.
...
"""
    ...
    def _detect_site_used(self, history: Any) -> Optional[str]:
        """Scan visited URLs (reverse) to identify which preferred site yielded the result."""
        try:
            ...
            for url in reversed(urls):
                ...
                for alias in PREFERRED_SITE_ALIASES:
                    if alias in u:
                        # normalize alias to canonical site if possible
                        for site in PREFERRED_SITES:
    ...
    async def check_single_domain(self, domain: str, attempt: int = 1) -> DomainCheckResult:
        console.print(f"\n{'='*70}")
        ...
        try:
            ...
            logger.info(f"Agent completed in {exec_time}s with {step_count} steps")
            ...
            # Show quick line incl. site used
            site_note = f" | Site: {result.site_used}" if result.site_used else ""
            console.print(f"[dim]Steps: {step_count} | Time: {exec_time}s{site_note}[/dim]")
    ...
    async def run_session(self) -> Dict[str, Any]:
        console.print(Panel.fit(
            "[bold white]SESSION 1: BASELINE (No ACE, Fixed Multi-Site Pipeline)[/bold white]\n"
        ...
        try:
            ...
            with Progress(
                ...
                task = progress.add_task(
                    "[cyan]Checking domains (fixed pipeline)...",
            ...
            # NEW: average steps per domain for proper Session 2 comparison
        ...
        try:
            output_data = {
                'metadata': {
                    ...
                    'description': 'Domain availability checker WITHOUT ACE (fixed multi-site pipeline)',
                    'model': self.model,
                    'sites_policy': {
                        'preferred': list(PREFERRED_SITES),
                        'avoided': list(AVOIDED_SITES),
    ...
    def print_summary(self) -> None:
        table = Table(title="SESSION 1 SUMMARY (Fixed Multi-Site Pipeline)", show_header=True,
